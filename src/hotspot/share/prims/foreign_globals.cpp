...
int ForeignGlobals::field_offset(InstanceKlass* cls, const char* fieldname, Symbol* sigsym) {
  TempNewSymbol fieldnamesym = SymbolTable::probe(fieldname, (int)strlen(fieldname));
  fieldDescriptor fd;
...
ForeignGlobals::ForeignGlobals() {
  ...
  TempNewSymbol symVMSArray = SymbolTable::probe(strVMSArray, (int)strlen(strVMSArray));
  assert(symVMSArray != nullptr, "Field not found");

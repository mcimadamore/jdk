...
import jdk.internal.foreign.Block;
 ...
 * requests in a similar way as a {@link SegmentAllocator#slicingAllocator(MemorySegment) slicing allocator}, as shown below:
 * <ul>
 ...
 */
public sealed class StackArena implements Arena {
    ...
    @ForceInline
        ...
        this.underlying = Arena.ofConfined();
        this.parent = parent;
    }

    void checkTop() {
        MemorySessionImpl.toMemorySession(underlying).checkValidState();
        if (!isTop) {
     ...
     * access to the returned segment is only possible from the thread owning this arena.
    ...
    public MemorySegment allocate(long bytesSize, long bytesAlignment) {
        ...
        checkTop();
        return currentBlock.nextSlice(bytesSize, bytesAlignment, this);
    }
    ...
    static final class Root extends StackArena {
        ...
        public Root(long blockSize, long arenaSize) {
            super(null);
            blocks = new Block(blockSize, arenaSize, underlying);

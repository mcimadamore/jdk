 ...
 *     {@link MemorySegment#expand(long) resizing} the segment first).</li>
 * </ul>
 *
...
public interface SymbolLookup {
    ...
    static SymbolLookup loaderLookup() {
        ...
        return name -> {
            ...
            return addr == 0L ?
                    Optional.empty() :
                    Optional.of(loaderScope.wrap(addr, null).expand(0L));
        };
    }
    ...
    private static <Z> SymbolLookup libraryLookup(Z libDesc, BiFunction<RawNativeLibraries, Z, NativeLibrary> loadLibraryFunc, NativeAllocator libScope) {
        ...
        return name -> {
            ...
            return addr == 0L ?
                    Optional.empty() :
                    Optional.of(libScope.wrap(addr, null).expand(0));
        };
    }
}

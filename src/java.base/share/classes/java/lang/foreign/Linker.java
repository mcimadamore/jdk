 ...
 * {@linkplain ValueLayout.OfAddress address layout} returns a native segment that is always alive.
 * Moreover, the size of the returned segment is {@code 0}.
 * <p>
 ...
 * is a native segment that is always alive.
 * Moreover, the size of this segment argument is {@code 0}.
 *
 * @implSpec
...
public sealed interface Linker permits AbstractLinker {
     ...
     * allocator. Calling such a function pointer from foreign code will result in the execution of the provided
     * method handle.
     * <p>
     * The returned memory segment's address points to the newly allocated upcall stub. The segment is obtained using
     * the provided allocator. As such, the corresponding upcall stub will be deallocated
     ...
     * @param allocator the allocator used to allocate the returned upcall stub segment.

...
final class LazyUpdater<R, X> {
    ...
    private final Function<? super R, ? extends X> computer;

    static <R, X> LazyUpdater<R, X> ofInstance(Class<R> declaringClass,
                                                 String fieldName,
                                                 Class<?> fieldType,
                                                 Function<? super R, ? extends X> computer) {
        return ofInstance(declaringClass, fieldName, fieldType, MethodHandles.publicLookup(), computer);
    }
    ...
    static <R, X> LazyUpdater<R, X> ofInstance(Class<R> declaringClass,
                                                 ...
                                                 Function<? super R, ? extends X> computer) {
        return new LazyUpdater<>(declaringClass, fieldName, fieldType, lookup, false, computer);
    }

    static <X> LazyUpdater<Void, X> ofStatic(Class<?> declaringClass,
                                               String fieldName,
                                               Class<?> fieldType,
                                               Function<? super Void, ? extends X> computer) {
        return ofStatic(declaringClass, fieldName, fieldType, MethodHandles.publicLookup(), computer);
    }
    ...
    static <X> LazyUpdater<Void, X> ofStatic(Class<?> declaringClass,
                                               ...
                                               MethodHandles.Lookup lookup,
                                               Function<? super Void, ? extends X> computer) {
        return new LazyUpdater<>(declaringClass, fieldName, fieldType, lookup, true, computer);
    }
    ...
    @ForceInline
    X getOrCompute(R receiver) {
        Object base = isStatic ? staticBase : receiver;
        X result = (X) UNSAFE.getReferenceAcquire(base, fieldOffset);
        if (result != null) {
            return result;
        }
        return getOrComputeSlow(base, receiver);
    }

    @SuppressWarnings("unchecked")
    X getOrComputeSlow(Object base, R receiver) {

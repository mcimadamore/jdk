...
public class NativeEntryPoint {
    ...
    private record CacheKey(MethodType methodType, ABIDescriptor abi,
                            ...
                            boolean needsReturnBuffer, boolean trivial) {}
    ...
    public static NativeEntryPoint make(ABIDescriptor abi,
        ...
        CacheKey key = new CacheKey(methodType, abi, Arrays.asList(argMoves), Arrays.asList(returnMoves), needsReturnBuffer, trivial);
        return NEP_CACHE.get(key, k -> {
            long downcallStub = makeDowncallStub(methodType, abi, argMoves, returnMoves, needsReturnBuffer, trivial);

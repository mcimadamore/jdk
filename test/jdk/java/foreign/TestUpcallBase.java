...
public abstract class TestUpcallBase extends CallGeneratorHelper {
    ...
    @BeforeClass
    void setup() {
        DUMMY_STUB = ABI.upcallStub(DUMMY, FunctionDescriptor.ofVoid(), SegmentAllocator.auto());
    }
    ...
    static Object[] makeArgs(SegmentAllocator session, Ret ret, List<ParamType> params, List<StructFieldType> fields, List<Consumer<Object>> checks, List<Consumer<Object[]>> argChecks) throws ReflectiveOperationException {
        return makeArgs(session, ret, params, fields, checks, argChecks, List.of());
    }

    static Object[] makeArgs(SegmentAllocator session, Ret ret, List<ParamType> params, List<StructFieldType> fields, List<Consumer<Object>> checks, List<Consumer<Object[]>> argChecks, List<MemoryLayout> prefix) throws ReflectiveOperationException {
    ...
    static MemorySegment makeCallback(SegmentAllocator session, Ret ret, List<ParamType> params, List<StructFieldType> fields, List<Consumer<Object>> checks, List<Consumer<Object[]>> argChecks, List<MemoryLayout> prefix) {
        if (params.isEmpty()) {
            return DUMMY_STUB;
        }
    ...
    static Object passAndSave(Object[] o, AtomicReference<Object[]> ref, int retArg, List<MemoryLayout> layouts) {
        for (int i = 0; i < o.length; i++) {
            if (layouts.get(i) instanceof GroupLayout) {
                ...
                MemorySegment copy = SegmentAllocator.auto().allocate(ms.byteSize());
                copy.copyFrom(ms);
                o[i] = copy;
            }
        }

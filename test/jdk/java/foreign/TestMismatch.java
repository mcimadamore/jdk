...
public class TestMismatch {
    ...
    public void testEmpty() {
        ...
        try (Arena arena = Arena.openConfined()) {
            var nativeSegment = MemorySegment.allocateNative(4, 4, arena);;
    ...
    public void testLarge() {
        ...
        if (ValueLayout.ADDRESS.byteSize() > 32) {
            try (Arena arena = Arena.openConfined()) {
                var s1 = MemorySegment.allocateNative((long) Integer.MAX_VALUE + 10L, 8, arena);;
                var s2 = MemorySegment.allocateNative((long) Integer.MAX_VALUE + 10L, 8, arena);;
                assertEquals(s1.mismatch(s1), -1);
                assertEquals(s1.mismatch(s2), -1);
                assertEquals(s2.mismatch(s1), -1);
    ...
    public void testClosed() {
        MemorySegment s1, s2;
        try (Arena arena = Arena.openConfined()) {
            s1 = MemorySegment.allocateNative(4, 1, arena);;
            s2 = MemorySegment.allocateNative(4, 1, arena);;
        }
    ...
    public void testThreadAccess() throws Exception {
        try (Arena arena = Arena.openConfined()) {
            var segment = MemorySegment.allocateNative(4, 1, arena);;
            {

...
public class TestSegments {
    ...
    public void testBadAllocateAlign(long size, long align) {
        SegmentAllocator.auto().allocate(size, align);
    }

    @Test
    public void testZeroLengthNativeSegment() {
        try (Arena arena = Arena.openConfined()) {
            SegmentAllocator session = arena;
    ...
    public void testAllocateTooBig() {
        SegmentAllocator.auto().allocate(Long.MAX_VALUE);
    }

    @Test(expectedExceptions = OutOfMemoryError.class)
    public void testNativeAllocationTooBig() {
        MemorySegment segment = SegmentAllocator.auto().allocate(1024L * 1024 * 8 * 2); // 2M
    }
    ...
    public void testEqualsOffHeap() {
        try (Arena arena = Arena.openConfined()) {
            ...
            assertEquals(segment, SegmentAllocator.global().wrap(segment.address(), null).expand(100));
    ...
    public void testHashCodeOffHeap() {
        try (Arena arena = Arena.openConfined()) {
            ...
            assertEquals(segment.hashCode(), SegmentAllocator.global().wrap(segment.address(), null).expand(100).hashCode());
        }
    }
    ...
    public void testSmallSegmentMax() {
        ...
        MemorySegment memorySegment = SegmentAllocator.auto().allocate(10);
    ...
    public void testSmallSegmentMin() {
        ...
        MemorySegment memorySegment = SegmentAllocator.auto().allocate(10L);
        memorySegment.get(JAVA_INT, offset);
    }
    ...
    public void testSegmentOOBMessage() {
        try {
            var segment = SegmentAllocator.global().allocate(10);
    ...
    public Object[][] segmentFactories() {
        List<Supplier<MemorySegment>> l = List.of(
                ...
                () -> SegmentAllocator.auto().allocate(4L),
                () -> SegmentAllocator.auto().allocate(4L, 8),
                () -> SegmentAllocator.auto().allocate(JAVA_INT),
                () -> SegmentAllocator.auto().allocate(4L),
                () -> SegmentAllocator.auto().allocate(4L, 8),
                () -> SegmentAllocator.auto().allocate(JAVA_INT)

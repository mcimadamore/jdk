...
@Fork(value = 3, jvmArgsAppend = { "--enable-preview", "--enable-native-access=ALL-UNNAMED", "--add-opens=java.base/jdk.internal.misc=ALL-UNNAMED", "--add-modules=jdk.incubator.vector" })
public class BinarySearch extends JavaLayouts {
    ...
    private long mPage;
    private MemorySegment pageSegment;
    private MemorySegment parsedKeysSegment;
    private MemorySegment parsedLongsSegment;
    private int parsedKeyCount;
    ...
    public void setup() {
        ...
        pageSegment = MemorySegment.ofAddress(mPage).reinterpret(NODE_DATA.length, Arena.global(), null);
        key = KEY;
        MemorySegment.copy(NODE_DATA, 0, ALL, ValueLayout.JAVA_BYTE, mPage, NODE_DATA.length);

        int startPos = p_ushortGetLE(mPage, 8);
        int endPos = p_ushortGetLE(mPage, 10);
        parsedKeyCount = ((endPos - startPos) / 2) + 1;
        parsedKeysSegment = Arena.global().allocate((long) parsedKeyCount * KEY.length);
        parsedLongsSegment = Arena.global().allocate((long) parsedKeyCount * JAVA_LONG.byteSize());
        packedKey = packKey(KEY, 0);

        for (int entry = 0; entry < parsedKeyCount; entry++) {
            int vecPos = startPos + (entry << 1);
            int compareLoc = p_ushortGetLE(mPage, vecPos);
            for (int i = 0; i < KEY.length; i++) {
                parsedKeysSegment.set(JAVA_BYTE, (long) entry * KEY.length + i,
                                      p_byteGet(mPage, compareLoc + 1 + i));
            }
            long packedPageKey = packPageKey(mPage, compareLoc + 1);
            parsedLongsSegment.set(JAVA_LONG_UNALIGNED, (long) entry * JAVA_LONG.byteSize(), packedPageKey);
        }

        verifyParsedRepresentations();
    ...
    @Benchmark
    public int binarySearch_ffm_vanilla() {
        byte[] key = this.key;
        MemorySegment page = pageSegment;
        final int keyLen = key.length;
        final int startPos = p_bounded_searchVecStart(page);
        int lowPos = startPos;
        int highPos = p_bounded_searchVecEnd(page);
        ...
        outer: while (lowPos <= highPos) {
            ...
            int compareLoc = p_bounded_ushortGetLE(page, midPos);
            int compareLen = p_bounded_byteGet(page, compareLoc++);
            if (compareLen >= 0) {
                compareLen++;
            }
            ...
            int i = Math.min(lowMatch, highMatch);
            for (; i<minLen; i++) {
                byte cb = p_bounded_byteGet(page, compareLoc + i);
                byte kb = key[i];
                if (cb != kb) {
                    if ((cb & 0xff) < (kb & 0xff)) {
                        lowPos = midPos + 2;
    ...
    @Benchmark
    public int binarySearch_ffm_reinterpret() {
        byte[] key = this.key;
        long page = mPage;
        final int keyLen = key.length;
        final int startPos = p_per_access_searchVecStart(page);
        int lowPos = startPos;
        int highPos = p_per_access_searchVecEnd(page);
        ...
        outer: while (lowPos <= highPos) {
            ...
            int compareLoc = p_per_access_ushortGetLE(page, midPos);
            int compareLen = p_per_access_byteGet(page, compareLoc++);
            if (compareLen >= 0) {
                compareLen++;
            }
            ...
            int i = Math.min(lowMatch, highMatch);
            for (; i<minLen; i++) {
                byte cb = p_per_access_byteGet(page, compareLoc + i);
                byte kb = key[i];
                if (cb != kb) {
                    if ((cb & 0xff) < (kb & 0xff)) {
                        lowPos = midPos + 2;
    ...
    @Benchmark
    public int binarySearch_ffm_parsed_keys() {
        byte[] key = this.key;
        MemorySegment keys = parsedKeysSegment;
        int low = 0;
        int high = parsedKeyCount - 1;

        while (low <= high) {
            int mid = (low + high) >>> 1;
            long base = (long) mid * key.length;

            int i = 0;
            for (; i < key.length; i++) {
                byte cb = keys.get(JAVA_BYTE, base + i);
                byte kb = key[i];
                if (cb != kb) {
                    if ((cb & 0xff) < (kb & 0xff)) {
                        low = mid + 1;
    ...
    @Benchmark
    public int binarySearch_ffm_parsed_keys_batch() {
    ...
    @Benchmark
    public int binarySearch_unsafe_parsed_keys_batch() {
        long page = parsedLongsSegment.address();
        long key = packedKey;
        int low = 0;
        int high = parsedKeyCount - 1;
        ...
        while (low <= high) {
            int mid = (low + high) >>> 1;
            long candidate = UNSAFE.getLongUnaligned(null, page + ((long) mid << 3));
    ...
    @Benchmark
    public int binarySearch_ffm_parsed_keys_batch_vector_tail() {
        MemorySegment keys = parsedLongsSegment;
        long key = packedKey;
        int low = 0;
    ...
    static long packKey(byte[] key, int offset) {
        return ((long) Byte.toUnsignedInt(key[offset]) << 56)
                ...
                | ((long) Byte.toUnsignedInt(key[offset + 2]) << 40)
                | ((long) Byte.toUnsignedInt(key[offset + 3]) << 32)
                | ((long) Byte.toUnsignedInt(key[offset + 4]) << 24)
                | ((long) Byte.toUnsignedInt(key[offset + 5]) << 16)
                | ((long) Byte.toUnsignedInt(key[offset + 6]) << 8)
                | Byte.toUnsignedInt(key[offset + 7]);
    }

    static long packPageKey(long page, int offset) {
        return ((long) Byte.toUnsignedInt(p_byteGet(page, offset)) << 56)
                | ((long) Byte.toUnsignedInt(p_byteGet(page, offset + 1)) << 48)
                | ((long) Byte.toUnsignedInt(p_byteGet(page, offset + 2)) << 40)
                | ((long) Byte.toUnsignedInt(p_byteGet(page, offset + 3)) << 32)
                | ((long) Byte.toUnsignedInt(p_byteGet(page, offset + 4)) << 24)
                | ((long) Byte.toUnsignedInt(p_byteGet(page, offset + 5)) << 16)
                | ((long) Byte.toUnsignedInt(p_byteGet(page, offset + 6)) << 8)
                | Byte.toUnsignedInt(p_byteGet(page, offset + 7));
    }

    void verifyParsedRepresentations() {
        for (int i = 1; i < parsedKeyCount; i++) {
            long prevLong = parsedLongsSegment.get(JAVA_LONG_UNALIGNED, (long) (i - 1) * JAVA_LONG.byteSize());
            long currLong = parsedLongsSegment.get(JAVA_LONG_UNALIGNED, (long) i * JAVA_LONG.byteSize());
            if (Long.compareUnsigned(prevLong, currLong) > 0) {
                throw new AssertionError("parsed long keys are not sorted at " + i);
            }
        }

        int vanilla = binarySearch_ffm_vanilla();
        int parsedBytes = binarySearch_ffm_parsed_keys();
    ...
    int p_bounded_searchVecStart(MemorySegment page) {
        return p_bounded_ushortGetLE(page, 8);
    }

    int p_bounded_searchVecEnd(MemorySegment page) {
        return p_bounded_ushortGetLE(page, 10);
    }

    static byte p_bounded_byteGet(MemorySegment page, int index) {
        return page.get(JAVA_BYTE, index);
    }

    static int p_bounded_ushortGetLE(MemorySegment page, int index) {
        return page.get(CHAR_LE, index);
    }

    int p_per_access_searchVecStart(long page) {
        return p_per_access_ushortGetLE(page, 8);
    }

    int p_per_access_searchVecEnd(long page) {
        return p_per_access_ushortGetLE(page, 10);
    }

    static byte p_per_access_byteGet(long page, int index) {
        return MemorySegment.ofAddress(page + index).reinterpret(1).get(JAVA_BYTE, 0);
    }

    static int p_per_access_ushortGetLE(long page, int index) {
        return MemorySegment.ofAddress(page + index).reinterpret(2).get(CHAR_LE, 0);
    }
